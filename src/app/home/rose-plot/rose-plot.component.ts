...
export class RosePlotComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  setChart() {
    ...
    this.graph = {
      ...
      layout: {
        ...
        dragmode: false,
        autosize: false,
        margin: { t: 144, r: 144, b: 144, l: 144 },
        polar: {
          radialaxis: {
            visible: true,
...
// Viridis      => perceptually uniform, great general-purpose scale
// Cividis      => colorblind-friendly version of Viridis
// Plasma       => vivid purple→yellow smooth gradient
// Inferno      => dark purple→orange/yellow, high contrast
// Magma        => dark→purple→white, soft contrast
// Turbo        => balanced rainbow (good visibility)
// Hot          => black→red→yellow→white (thermal style)
// Electric     => dark blue→magenta→yellow (neon look)

// // 🌈 Multi-hue / rainbow-like
// Rainbow      => full rainbow spectrum
// Jet          => classic MATLAB rainbow (blue→red)
// Portland     => pastel, cooler feel
// Balance      => diverging red↔blue (neutral center)

// // ⚖️ Diverging (two hues split around midpoint)
// RdBu         => red↔blue
...
// // 🧊 Cool / Warm and Earthy
// Bluered      => dark blue↔red (simple diverging)
// Earth        => muted earthy tones (natural look)
// Ice          => cyan→light gray
// YlGnBu       => yellow→green→blue (smooth cool gradient)

// // 🧪 Specialized / Misc
// HSV          => full hue wheel (wraps colors)
// Greys        => grayscale
// Picnic       => bright pastel rainbow
// Blackbody    => physics-style thermal (black→red→yellow→white)
// Electric     => glowing neon tone

// // 🧬 Scientific-grade perceptually uniform sets
// Viridis, Cividis, Plasma, Inferno, Magma => perceptually uniform, ideal for data visualization
